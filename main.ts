...
class User {
    ...
    cartTotal() {
        ...
        const shipping = Math.round((subtotal * 0.10) * 100) / 100;
...
router.post("/signup", (req, res) => {
    ...
    res.status(201).json({
        msg: "Success",
        data: { id: user.id, email: user.email, name: user.name, username: user.email }
    });
});
...
router.post("/login", (req, res) => {
    ...
    if (!user) {
        res.status(404).json({ status: 404, msg: "No user with this email exists"});
        return;
...
router.get("/checkout/:userId/summary", (req, res) => {
    ...
    if (!user) {
        res.status(404).json({ status: 404, msg: "No user with this ID exists"});
        return;
    }

    res.status(200).json({ msg: "Success", data: { items: user.cartJson(), ...user.cartTotal() } });
});
...
cartRouter.post("/:userId", (req, res) => {
    ...
    user.cart.push({ product, quantity: data.data.quantity, id: ++user.cartId });
...
cartRouter.delete("/:userId/:itemId", (req, res) => {
    ...
    const itemIndex = user.cart.findIndex(item => item.id === +req.params.itemId);
    if (itemIndex === -1) {
        res.status(404).json({ status: 404, msg: "Item does not exist"});
        return;
    }
...
ordersRouter.post("/:userId", (req, res) => {
    ...
    res.status(200).json({ msg: "Success", data: { id: user.orders.at(-1)!.id } });
});
